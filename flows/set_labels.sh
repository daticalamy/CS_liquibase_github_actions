...
status_output=$(liquibase status --log-level=OFF)
...
echo "$status_output" | tail -n +2 | while IFS= read -r line; do
	...
	liquibase set-labels --log-level=OFF --set-as=$issue_id --changeset-author=$author --changeset-id=$id --changeset-path=$filepath
